...
class FetchSlide(embodied.Env):
    """Wrapped version of the FetchSlide environment with image observations."""
    ...
    def step(self, action):
        ...
        block_xy = self._env._utils.get_joint_qpos(
            self._env.model, self._env.data, 'object0:joint'
        )[:2]
    ...
    def _viewer_setup(self):
        self._env._viewer_setup()
        self._env.viewer.cam.lookat[Ellipsis] = np.array([1.35, 0.8, 0.4])
        self._env.viewer.cam.distance = 1.75
        self._env.viewer.cam.azimuth = 90
        self._env.viewer.cam.elevation = -40
